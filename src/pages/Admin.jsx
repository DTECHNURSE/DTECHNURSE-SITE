...
import { Loader2, Save, LinkIcon, Bold, Italic, List, Image, LogOut, Code, Pencil, Trash2, Plus, ArrowLeft } from 'lucide-react';
...
export default function Admin() {
  ...
  // --- 2. LOGIN SCREEN (uses AdminLogin component with Google + Email/Password) ---
  if (!user) {
        ...
        <div className="section-container py-12 md:py-16 max-w-4xl mx-auto">
          ...
          {isLoadingPosts ? (
            ...
            <div className="text-center py-16 bg-gray-50 rounded-xl border border-brand-border">
              ...
              <button onClick={handleCreateNew} className="text-brand-blue font-semibold mt-2 hover:underline">
              ...
              {posts.map((post) => (
                <div key={post.id} className="flex items-center gap-4 p-4 bg-white border border-brand-border rounded-xl hover:shadow-sm transition-shadow">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleEditPost(post)} className="p-2 text-brand-blue hover:bg-brand-light rounded-lg transition-colors" title="Edit">
                      <Pencil size={18} />
                    </button>
                    <button onClick={() => handleDeletePost(post.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
      ...
      <div className="section-container py-12 md:py-16 max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-4">
            <button onClick={() => setView('list')} className="text-brand-gray hover:text-brand-dark">
              <ArrowLeft size={24} />
            </button>
            <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 md:p-10 rounded-2xl border border-brand-border shadow-sm">
          ...
          <div>
            ...
            <input
              type="url" name="coverUrl" value={formData.coverUrl} onChange={handleChange}
          ...
          <div>
            ...
            <textarea name="excerpt" value={formData.excerpt} onChange={handleChange} rows="2"
              ...
              placeholder="A brief summary..." required></textarea>
          </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-brand-border">
            ...
            <button type="submit" disabled={isSubmitting}
              ...
              <Save size={18} /> {isSubmitting ? 'Saving...' : (editingId ? 'Update Post' : 'Publish Post')}
            </button>
          </div>
        </form>
      </div>
